...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      navigate(
        result.role === "teacher" ? "/teacher/students" : "/student/home",
      );
    } catch (err) {
    ...
    <div className="flex justify-center items-center bg-gradient-to-br from-slate-100 dark:from-slate-900 via-slate-50 dark:via-slate-800 to-brand-100 dark:to-slate-900 p-4 min-h-screen">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white/80 dark:bg-slate-800/90 shadow-xl backdrop-blur-sm p-7 rounded-3xl">
          ...
          <h2 className="mb-1 font-poppins font-bold text-slate-800 dark:text-white text-2xl text-center">
            Welcome Back
          </h2>
          <p className="mb-6 text-slate-500 dark:text-slate-400 text-sm text-center">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="relative">
              ...
              <input
                type="email"
                placeholder={
                  role === "student" ? "Your email" : "Teacher email"
                }
                value={email}
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex justify-center items-center gap-1.5 bg-gradient-to-r from-brand-500 hover:from-brand-600 to-brand-600 hover:to-brand-700 disabled:opacity-50 shadow-lg py-4 rounded-2xl w-full font-extrabold text-white text-base active:scale-[0.98] transition-all cursor-pointer"
            >
              {loading ? (
                "Signing in..."
              ) : (
          ...
          {/* Info for student */}
          {role === "student" && (
            <div className="bg-gradient-to-r from-brand-50 to-green-50 dark:from-slate-700/70 dark:to-slate-700/40 mt-6 px-3 py-3 border border-brand-100/70 dark:border-brand-400/20 rounded-2xl">
              <p className="flex justify-center items-center gap-2.5 font-bold text-brand-700 dark:text-brand-300 text-xs text-center whitespace-nowrap">
                <FaGraduationCap className="flex-shrink-0 text-sm" />
                <span>Your login credentials were given by your teacher</span>
              </p>
            </div>
          )}
        </div>
        ...
        <div className="flex justify-center items-center gap-3 mt-2">
          <Link
            to="/terms"
            className="text-slate-400 hover:text-brand-500 dark:text-slate-500 text-xs transition-colors"
          >
            Terms of Service
          </Link>
          <span className="text-slate-300 dark:text-slate-600 text-xs leading-none">
            •
          </span>
          <Link
            to="/privacy"
