...
const BLANK_FORM = {
  ...
  maxCoins: 20,
  timeLimit: 10,
  questions: [JSON.parse(JSON.stringify(BLANK_Q))],
};
const SUBJECTS = [
  "Math",
  "English",
...
export default function TeacherQuizzesPage() {
  ...
  const addQ = () => {
    ...
    setForm((f) => ({
      ...f,
      questions: [...f.questions, JSON.parse(JSON.stringify(BLANK_Q))],
    }));
  };
  const removeQ = (i) => {
  ...
  const handleSave = async () => {
    ...
    for (let i = 0; i < form.questions.length; i++) {
      if (!form.questions[i].question.trim()) {
        showToast(`❌ ${i + 1}-savol bo'sh`, "error");
        return;
      }
      if (form.questions[i].options.some((o) => !o.trim())) {
        showToast(`❌ ${i + 1}-savol variantlari to'liq emas`, "error");
        return;
      }
    }
    setSaving(true);
    try {
      ...
      setView("list");
      setForm(JSON.parse(JSON.stringify(BLANK_FORM)));
      loadQuizzes();
    } catch (err) {
  ...
  const handleToggle = async (id) => {
    const r = await fetch(`${API_BASE_URL}/quizzes/${id}/toggle`, {
      method: "PATCH",
      headers: { Authorization: `Bearer ${token}` },
    });
    const d = await r.json();
    ...
    return (
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-5 md:py-0 max-w-7xl">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <p className="mb-1 font-extrabold text-[11px] text-indigo-400 uppercase tracking-widest">
              Teacher
            </p>
            <h1 className="font-black text-slate-900 dark:text-white text-3xl md:text-4xl tracking-tight">
              Quizzes
            </h1>
          </div>
          <button
        ...
        <div className="gap-3 grid grid-cols-3 mb-6">
          ...
          <Stat
            label="Active"
            value={quizzes.filter((q) => q.active).length}
            color="bg-emerald-500 text-white"
          />
          <Stat
            label="Questions"
            value={quizzes.reduce((a, q) => a + (q.questions?.length || 0), 0)}
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            <div className="mb-4">
               <img src="/icon.png" alt="Loading" className="w-12 h-12 animate-bounce" />
            </div>
          ...
          <div className="gap-3 grid grid-cols-1 md:grid-cols-2">
            {quizzes.map((quiz, i) => (
              <div
                key={quiz._id}
                className="group bg-white dark:bg-slate-800 shadow-sm hover:shadow-md hover:shadow-slate-200/50 dark:hover:shadow-slate-900/50 p-5 rounded-2xl transition-all"
              >
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      <span className="font-black text-[15px] text-slate-900 dark:text-white truncate leading-tight">
                        {quiz.title}
                      </span>
                      <span
                        className={`text-[9px] font-extrabold px-2 py-0.5 rounded-full uppercase ${quiz.active ? "bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-400" : "bg-slate-100 dark:bg-slate-700 text-slate-400"}`}
                      >
                    ...
                    <div className="flex flex-wrap gap-x-3 gap-y-0.5 font-medium text-[11px] text-slate-400 dark:text-slate-500">
                      ...
                      <span className="font-bold text-brand-500 flex items-center gap-1">
                        <Icon name="coins" size={10} /> {quiz.maxCoins} max
                      </span>
                    </div>
                  </div>
                </div>
                <div className="gap-2 grid grid-cols-3">
                  <button
                    ...
                    className="col-span-1 flex items-center justify-center gap-1.5 bg-brand-50 hover:bg-brand-100 dark:bg-brand-900/30 dark:hover:bg-brand-800 py-2 border-none rounded-xl font-bold text-[11px] text-brand-600 dark:text-brand-400 transition-colors cursor-pointer"
                  >
                    <Icon name="chart" size={12} /> Results
                  </button>
                  <button
                    onClick={() => handleToggle(quiz._id)}
                    className={`col-span-1 flex items-center justify-center gap-1.5 py-2 text-[11px] font-bold rounded-xl border-none cursor-pointer transition-colors ${quiz.active ? "bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-800" : "bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-800"}`}
                  >
                    {quiz.active ? <><Icon name="pause" size={10} /> Pause</> : <><Icon name="play" size={10} /> Start</>}
                  </button>
                  <button
                    onClick={() => handleDelete(quiz._id, quiz.title)}
                    className="col-span-1 flex items-center justify-center gap-1.5 bg-red-50 hover:bg-red-100 dark:bg-red-900/30 dark:hover:bg-red-800 py-2 border-none rounded-xl font-bold text-[11px] text-red-400 transition-colors cursor-pointer"
                  >
  ...
  return (
    <div className="mx-auto px-4 sm:px-6 lg:px-8 py-5 md:py-0 pb-12 max-w-7xl">
      <div className="flex items-center gap-3 mb-7">
        ...
        <div>
          <h2 className="font-black text-slate-900 dark:text-white text-2xl tracking-tight">
            Yangi Test
          </h2>
          <p className="text-[11px] text-slate-400 dark:text-slate-500">
            {form.questions.length} ta savol
          </p>
        </div>
      </div>
      ...
      <div className="flex md:flex-row flex-col gap-6">
        ...
        <div className="flex-shrink-0 space-y-4 w-full md:w-72">
          ...
          <div className="gap-3 grid grid-cols-2">
            <div className="bg-violet-50 dark:bg-violet-900/30 p-4 border border-violet-100 dark:border-violet-800 rounded-2xl">
              <p className="mb-1 font-extrabold text-[10px] text-violet-400 uppercase tracking-wider flex items-center gap-1">
                Max <Icon name="coins" size={10} />
              </p>
              <input
                type="number"
                value={form.maxCoins}
                ...
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    maxCoins: parseInt(e.target.value) || 20,
                  }))
                }
            ...
            <div className="bg-sky-50 dark:bg-sky-900/30 p-4 border border-sky-100 dark:border-sky-800 rounded-2xl">
              <p className="mb-1 font-extrabold text-[10px] text-sky-400 uppercase tracking-wider flex items-center gap-1">
                <Icon name="history" size={10} /> min
              </p>
              <input
                type="number"
          ...
          <Section label="Coin jadvali">
            <div className="space-y-3">
              {[
                ...
                {
                  pct: 0,
                  color: "bg-gradient-to-br from-slate-200 to-slate-300",
                },
              ].map(({ pct, color }) => (
                <div key={pct} className="flex items-center gap-3">
                  <span
                    className={`w-7 h-7 flex items-center justify-center ${color} rounded-full text-white`}
                  >
                  ...
                  <span className="w-8 font-bold text-[10px] text-slate-400 dark:text-slate-500 text-right">
                    {pct}%
                  </span>
          ...
          >
            {saving
              ? "Saqlanmoqda..."
              : `✅ Saqlash (${form.questions.length} savol)`}
          </button>
        </div>
        ...
        <div className="flex-1 space-y-4">
          {form.questions.map((q, qi) => (
            <div
              key={qi}
              className="bg-white dark:bg-slate-800 p-5 border border-slate-100 hover:border-indigo-200 dark:border-slate-700 dark:hover:border-indigo-500 rounded-2xl transition-colors"
            >
              ...
              <div className="space-y-2">
                {q.options.map((opt, oi) => (
                  ...
                  >
                    <span
                      className={`w-6 h-6 rounded-lg flex items-center justify-center text-[11px] font-black flex-shrink-0 transition-colors ${q.correct === oi ? "bg-emerald-500 text-white" : "bg-white dark:bg-slate-600 border border-slate-200 dark:border-slate-500 text-slate-400"}`}
                    >
                      {OPT_LABELS[oi]}
                    </span>
                    <input
                      type="text"
                      placeholder={`${OPT_LABELS[oi]} variant *`}
                      value={opt}
                    ...
                    />
                    {q.correct === oi && (
                      <span className="flex-shrink-0 font-extrabold text-[11px] text-emerald-500">
                        ✅
                      </span>
                    )}
                  </div>
...
const Stat = ({ label, value, color }) => (
  <div className={`${color} rounded-2xl p-4 text-center`}>
    ...
    <p className="opacity-70 mt-0.5 font-bold text-[10px] uppercase tracking-wider">
      {label}
    </p>
  </div>
);
...
const EmptyState = ({ onNew }) => (
  <div className="flex flex-col justify-center items-center py-20 text-center">
    <div className="flex justify-center items-center bg-indigo-50 dark:bg-indigo-900/30 mb-5 rounded-3xl w-20 h-20">
      <img src="/icon.png" alt="Quiz Icon" className="w-12 h-12" />
    </div>
    <p className="mb-1 font-black text-slate-800 dark:text-white text-xl">
      Hali test yo'q
    </p>
    <p className="mb-6 text-slate-400 dark:text-slate-500 text-sm">
