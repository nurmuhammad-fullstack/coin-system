...
export default function TeacherAnalyticsPage() {
  ...
  const loadAnalytics = useCallback(async () => {
    setLoading(true);
    try {
      const [overviewData, quizData, studentData, coinData, shopData] =
        await Promise.all([
        ...
        <div className="space-y-5">
          ...
          <div className="gap-3 grid grid-cols-2 md:grid-cols-4">
            <div className="bg-gradient-to-br from-indigo-500 to-indigo-700 p-5 rounded-2xl text-white">
              ...
              <p className="opacity-80 mb-1 font-bold text-xs uppercase tracking-wider">
                Students
              </p>
            ...
            <div className="bg-gradient-to-br from-amber-500 to-amber-600 p-5 rounded-2xl text-white">
              ...
              <p className="font-poppins font-black text-3xl">
                {overview.totalCoinsDistributed?.toLocaleString() || 0}
              </p>
            </div>
          ...
          <div className="gap-3 grid grid-cols-2 md:grid-cols-4">
            ...
            <div className="bg-white dark:bg-slate-800 p-5 border border-slate-100 dark:border-slate-700 rounded-2xl">
              ...
              <p className="font-poppins font-black text-slate-800 dark:text-white text-2xl">
                {overview.totalCoinsSpent?.toLocaleString() || 0}
              </p>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 p-5 border border-slate-100 dark:border-slate-700 rounded-2xl">
              ...
              <p className="font-poppins font-black text-slate-800 dark:text-white text-2xl">
                {overview.totalStudents > 0
                  ? Math.round(
                      (overview.totalAttempts / overview.totalStudents) * 100,
                    )
                  : 0}
                %
              </p>
            ...
            <div className="bg-white dark:bg-slate-800 p-5 border border-slate-100 dark:border-slate-700 rounded-2xl">
              ...
              <div className="space-y-3 mt-3">
                {quizStats.slice(0, 5).map((quiz) => (
                  <div
                    key={quiz._id}
                    className="flex items-center gap-4 bg-slate-50 dark:bg-slate-700/50 p-3 rounded-xl"
                  >
                    <div className="flex-1 min-w-0">
                      <p className="font-extrabold text-slate-800 dark:text-white text-sm truncate">
                        {quiz.title}
                      </p>
                    ...
                    <div className="text-right">
                      <p className="font-black text-indigo-500 text-lg">
                        {quiz.avgScore}%
                      </p>
                      <p className="text-slate-400 dark:text-slate-500 text-xs">
                        avg score
        ...
        <div className="space-y-5">
          {quizStats.length === 0 ? (
            <div className="py-12 text-slate-400 dark:text-slate-500 text-center">
              ...
              <p className="font-bold dark:text-slate-400 text-sm">
                No quizzes yet
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              {quizStats.map((quiz) => (
                <div
                  key={quiz._id}
                  className="bg-white dark:bg-slate-800 p-5 border border-slate-100 dark:border-slate-700 rounded-2xl"
                >
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${quiz.active ? "bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400" : "bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400"}`}
                        >
                    ...
                    <div className="text-right">
                      <p className="font-black text-indigo-500 text-2xl">
                        {quiz.avgScore}%
                      </p>
                      <p className="text-slate-400 dark:text-slate-500 text-xs">
                        average score
                  ...
                  {studentStats.slice(0, 5).map((student, index) => (
                    ...
                    >
                      <span className="w-6 font-black text-center">
                        {index === 0 ? (
                          <FaTrophy className="mx-auto text-yellow-500" />
                        ) : index === 1 ? (
                          <FaMedal className="mx-auto text-gray-400" />
                        ) : index === 2 ? (
                          <FaMedal className="mx-auto text-amber-600" />
                        ) : (
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                      {studentStats.map((student) => (
                        ...
                        >
                          ...
                          <td className="py-3">
                            <span
                              className={`font-bold ${student.avgScore >= 70 ? "text-green-500" : student.avgScore >= 50 ? "text-amber-500" : "text-red-500"}`}
                            >
                              {student.avgScore}%
                            </span>
                          </td>
                          ...
                          <td className="py-3 font-medium text-red-500">
                            -{student.coinsSpent}
                          </td>
                ...
                {coinStats.recentTransactions?.map((tx) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-10 h-10 rounded-xl flex items-center justify-center ${tx.type === "earn" ? "bg-green-100 dark:bg-green-900/30" : "bg-red-100 dark:bg-red-900/30"}`}
                      >
                        {tx.type === "earn" ? (
                          <FaArrowUp className="text-green-500" />
                        ) : (
                    ...
                    <p
                      className={`font-black ${tx.type === "earn" ? "text-green-500" : "text-red-500"}`}
                    >
                      {tx.type === "earn" ? "+" : "-"}
                      {Math.abs(tx.amount)}
                    </p>
            ...
            <div className="space-y-3 mt-3">
              {shopStats.topItems?.map((item, index) => (
                <div
                  ...
                  className="flex items-center gap-4 bg-slate-50 dark:bg-slate-700/50 p-3 rounded-xl"
                >
                  <span className="w-6 font-black text-slate-400 text-center">
                    #{index + 1}
                  </span>
              ...
              {shopStats.recentPurchases?.map((purchase, index) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-slate-400 dark:text-slate-500 text-xs">
                      {purchase.student?.name} • Class {purchase.student?.class}
                    </p>
                  </div>
