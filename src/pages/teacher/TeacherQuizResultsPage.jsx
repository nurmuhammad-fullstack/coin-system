...
export default function TeacherQuizResultsPage() {
  ...
  if (loading)
    return (
      <div className="p-6 font-bold text-slate-400 dark:text-slate-500 text-center">
        Yuklanmoqda...
      </div>
    );
  if (!quiz)
  ...
  return (
    <div className="space-y-5 mx-auto px-4 sm:px-6 lg:px-8 py-5 md:py-0 max-w-7xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="font-poppins font-black text-slate-800 dark:text-white text-xl">
            {quiz.title}
          </h2>
          <p className="text-slate-400 dark:text-slate-500 text-xs">
            {quiz.questions?.length} savol â€¢ max ðŸª™{quiz.maxCoins}
          </p>
        </div>
      </div>
      ...
      <div className="gap-3 grid grid-cols-3">
        <div className="bg-violet-500 p-4 rounded-2xl text-white text-center">
          <p className="opacity-80 mb-1 font-bold text-[10px] uppercase">
            Ishtirokchi
          </p>
      ...
      {results.length === 0 ? (
        ...
        <div className="space-y-3">
          <p className="font-extrabold text-slate-400 dark:text-slate-500 text-xs uppercase tracking-wider">
            Natijalar
          </p>
          {results.map((r, i) => {
            ...
            return (
              <div
                key={r._id}
                className="flex items-center gap-3 bg-white dark:bg-slate-800 p-4 rounded-2xl"
              >
                <span className="w-7 font-black text-center">
                  {medals[i] || `#${i + 1}`}
                </span>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-slate-400 dark:text-slate-500 text-xs">
                    â± {Math.floor((r.timeTaken || 0) / 60)}:
                    {String((r.timeTaken || 0) % 60).padStart(2, "0")} min
                  </p>
                </div>
                <div className="text-right">
                  <p
                    className={`font-poppins font-black text-lg ${scoreColor}`}
                  >
                    {r.score}%
                  </p>
                  <p className="font-bold text-brand-600 dark:text-brand-400 text-xs">
                    +ðŸª™{r.coinsEarned}
                  </p>
                </div>
              </div>
