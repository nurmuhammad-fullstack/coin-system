...
export default function TeacherShopPage() {
  ...
  const items =
    catFilter === "All"
      ? shopItems
      : shopItems.filter((i) => i.category === catFilter);
  ...
  const handleImageUpload = (e) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      showToast("❌ Rasm 2MB dan kichik bo'lishi kerak", "error");
      return;
    }
    const reader = new FileReader();
    reader.onload = (ev) =>
      setForm((f) => ({ ...f, image: ev.target.result, emoji: null }));
    reader.readAsDataURL(file);
  };

  const handleAdd = async () => {
    if (!form.name.trim() || !form.cost) {
      showToast("❌ Nom va narxni kiriting", "error");
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    <div className="space-y-4 mx-auto px-4 sm:px-6 lg:px-8 py-5 md:py-0 max-w-7xl">
      ...
      <div className="gap-3 grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {item.tag && (
                  <span
                    className={
                      "text-[9px] font-extrabold px-2 py-0.5 rounded-full text-white " +
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <span className="font-medium text-[10px] text-slate-400 dark:text-slate-500">
                  {item.category}
                </span>
        ...
        {items.length === 0 && (
          <div className="col-span-2 py-10 text-slate-400 dark:text-slate-500 text-center">
            ...
            <p className="font-medium dark:text-slate-400 text-sm">
              No items yet
            </p>
          </div>
        )}
      ...
      {showModal && (
        ...
        >
          <h3 className="mb-4 font-poppins font-black text-slate-800 dark:text-white text-xl">
            Add Shop Item
          </h3>

          {/* Image upload */}
          <div className="mb-4">
            <p className="mb-2 font-bold text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider">
              Rasm yuklash
            </p>
            <div
            ...
            >
              {form.image ? (
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-slate-400 dark:text-slate-500 text-xs">
                    JPG, PNG • max 2MB
                  </p>
                </div>
              )}
              ...
              <div className="flex flex-wrap gap-2">
                {EMOJIS.map((e) => (
                  <button
                    ...
                    className={
                      "w-9 h-9 rounded-xl text-xl border-2 cursor-pointer transition-all " +
                      (form.emoji === e
