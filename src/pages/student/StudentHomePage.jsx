    ...
    <div className="space-y-5 mx-auto px-4 sm:px-6 lg:px-8 py-5 md:py-0 max-w-7xl">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <h2 className="font-poppins font-black text-slate-900 dark:text-white text-2xl md:text-3xl">
            Hi, {fn} 👋
          </h2>
        </div>
      ...
      <div className="bg-gradient-to-br from-brand-500 to-brand-700 p-6 rounded-3xl text-white">
        <p className="opacity-80 mb-3 font-bold text-xs uppercase tracking-widest">
          Total Balance
        </p>
        <div className="flex items-center gap-3 mb-4">
          <FaCoins className="text-4xl" />
          <span className="font-poppins font-black text-5xl md:text-6xl">
      ...
      <Card className="p-5">
        <div className="flex justify-between items-center mb-4">
          ...
          <button
            ...
            className="bg-transparent border-none font-bold text-brand-500 dark:text-brand-400 text-xs cursor-pointer"
          >
            View all →
          </button>
        </div>
          ...
          <div className="space-y-1">
            {txs.map((tx) => {
              ...
              return (
                <div
                  key={tx._id || tx.id}
                  className="flex items-center gap-3 py-3 border-slate-100 dark:border-slate-700 border-b"
                >
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center text-sm flex-shrink-0 ${earn ? "bg-brand-50 dark:bg-brand-900/50 text-brand-600 dark:text-brand-400" : "bg-red-50 dark:bg-red-900/50 text-red-500 dark:text-red-400"}`}
                  >
                  ...
                  <span
                    className={`text-sm font-black ${earn ? "text-brand-600 dark:text-brand-400" : "text-red-500 dark:text-red-400"}`}
                  >
                    {earn ? "+" : ""}
                    {tx.amount}
                  </span>
