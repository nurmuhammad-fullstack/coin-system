  ...
  return (
    <div className="space-y-4 mx-auto px-4 sm:px-6 lg:px-8 py-5 max-w-7xl">
      <div className="flex justify-between items-center">
        <h2 className="font-poppins font-black text-gray-900 dark:text-white text-2xl">
          My Wallet
        </h2>
      ...
      <Card className="p-6 text-center">
        <p className="mb-2 font-extrabold text-slate-400 dark:text-slate-500 text-xs uppercase tracking-wider">
          Total Balance
        </p>
        <div className="flex justify-center items-center gap-2 mb-1">
          <FaCoins className="text-amber-500 text-2xl" />
          <span className="font-poppins font-black text-gray-900 dark:text-white text-5xl">
      ...
      <div className="gap-3 grid grid-cols-2">
        <Card className="p-4 text-center">
          ...
          <p className="font-black text-brand-400 text-xl">
            +{totalEarned.toLocaleString()}
          </p>
          <p className="mt-0.5 font-bold text-slate-500 text-xs">
            Total Earned
          </p>
        </Card>
        <Card className="p-4 text-center">
      ...
      <Card className="p-4">
        ...
        {txs.length === 0 && (
          <p className="py-6 text-slate-500 text-sm text-center">
            No transactions found
          </p>
        )}
        {txs.map((tx) => (
          <TxItem key={tx._id || tx.id} tx={tx} />
        ))}
      </Card>
    </div>
  );
