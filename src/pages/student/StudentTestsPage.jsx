...
export default function StudentTestsPage() {
  ...
  const availableQuizzes = quizzes.filter(
    (q) => q.active !== false && q.isActive !== false,
  );
      ...
      <div className="gap-3 grid grid-cols-3">
        {[
          ...
          <Card key={s.label} className={`${s.bg} p-4 border-none`}>
            ...
            <p className={`font-poppins font-black text-2xl ${s.color}`}>
              {s.value}
            </p>
            <p className="font-semibold text-slate-500 dark:text-slate-400 text-xs">
              {s.label}
      ...
      <Card className="p-5">
        ...
        {quizzesLoaded && availableQuizzes.length === 0 && (
          <div className="py-10 text-slate-400 dark:text-slate-500 text-center">
            ...
            <p className="mt-1 dark:text-slate-500 text-xs">
              Your teacher hasn't added any tests
            </p>
          </div>
        )}
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-4 p-4 rounded-2xl border transition-all ${
                      ...
                      ? "border-brand-100 dark:border-brand-900/50 bg-brand-50/40 dark:bg-brand-900/20"
                ...
                >
                  ...
                  {done ? (
                    <div className="flex-shrink-0 text-center">
                      ...
                      <p className="mt-1 font-medium text-[10px] text-slate-400 dark:text-slate-500">
                        1 attempt only
                      </p>
                    </div>
                  ) : (
