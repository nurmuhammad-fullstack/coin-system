...
export default function StudentQuizPage() {
  ...
  useEffect(() => {
    ...
    const done = quizAttempts?.find(
      (a) =>
        ...
        (a.studentId === currentUser?._id ||
          a.student === currentUser?._id ||
          (typeof a.student === "object" ? a.student?._id : a.student) ===
            currentUser?._id),
    );
    ...
    async (finalAnswers) => {
      ...
      try {
        ...
        finalAnswers.forEach((ans, i) => {
          if (questions[i] && Number(ans) === Number(questions[i].correct))
            correct++;
        });
  ...
  const handleSelect = (idx) => {
    if (!confirmed) setSelected(Number(idx));
  };
        ...
        <div className="bg-white shadow-2xl shadow-indigo-100 rounded-3xl w-full max-w-md overflow-hidden">
          <div className="bg-gradient-to-r from-slate-400 to-slate-500 p-8 text-white text-center">
            ...
            <p className="mt-1 text-slate-200 text-sm">
              Bu testni faqat bir marta yechish mumkin
            </p>
          </div>
          <div className="p-6">
            <div className="flex items-center gap-4 bg-slate-50 mb-4 p-4 rounded-2xl">
              ...
              <div>
                ...
                <p className="font-black text-indigo-600 text-2xl">
                  {result?.score ?? "–"}%
                </p>
              </div>
        ...
        <div className="w-full max-w-md">
          <div className="bg-white shadow-2xl shadow-indigo-100 rounded-3xl overflow-hidden">
            <div className="relative bg-gradient-to-r from-indigo-500 to-purple-600 p-8 overflow-hidden text-white text-center">
              ...
              {quiz.subject && (
                <p className="mt-1 font-medium text-indigo-200 text-sm">
                  {quiz.subject}
                </p>
              )}
            </div>
            <div className="p-6">
              <div className="gap-3 grid grid-cols-3 mb-5">
                {[
                  ...
                  <div
                    key={item.label}
                    className="bg-slate-50 p-3 rounded-2xl text-center"
                  >
                    <div className="mb-1 text-xl">{item.icon}</div>
                    <div className="font-black text-slate-800 text-lg leading-none">
                      {item.value}
                    </div>
                    <div className="mt-0.5 font-semibold text-[10px] text-slate-400">
                      {item.label}
                    </div>
                  </div>
                ))}
              ...
              <div className="flex items-start gap-3 bg-red-50 mb-5 p-4 border border-red-100 rounded-2xl">
                ...
                <div>
                  ...
                  <p className="mt-0.5 font-medium text-red-400 text-xs">
                    Testni boshlaganingizdan keyin qaytib bo'lmaydi.
                  </p>
                </div>
              </div>
              <button
  ...
  if (phase === "result") {
    ...
    const grade =
      score >= 90
        ? { emoji: "🏆", label: "A'lo!", color: "from-yellow-400 to-amber-500" }
        : score >= 70
          ? {
              emoji: "🌟",
              label: "Yaxshi!",
    ...
    return (
      <div className="flex justify-center items-center bg-gradient-to-br from-indigo-50 via-white to-purple-50 p-6 min-h-screen">
        <div className="w-full max-w-md">
          <div className="bg-white shadow-2xl shadow-indigo-100 rounded-3xl overflow-hidden">
            <div
              className={`bg-gradient-to-r ${grade.color} p-8 text-white text-center`}
            >
            ...
            <div className="p-6">
              <div className="gap-3 grid grid-cols-3 mb-5">
                {[
                  ...
                  },
                  {
                    icon: "❌",
                    label: "Noto'g'ri",
                    value: (result?.total ?? totalQ) - (result?.correct ?? 0),
                  ...
                  >
                    ...
                    <div className="opacity-70 mt-0.5 font-semibold text-[10px]">
                      {s.label}
                    </div>
                  </div>
                ))}
              ...
              <div className="flex items-center gap-4 bg-gradient-to-r from-amber-50 to-yellow-50 mb-5 p-4 border border-amber-200 rounded-2xl">
                ...
                <div>
                  ...
                  <p className="font-medium text-amber-500 text-xs">
  ...
  return (
    <div className="flex flex-col bg-gradient-to-br from-slate-50 to-indigo-50 min-h-screen">
      ...
      <div className="top-0 z-10 sticky flex items-center gap-4 bg-white shadow-sm px-4 py-3 border-slate-100 border-b">
        <div className="relative flex-shrink-0 w-12 h-12">
          ...
          <div className="absolute inset-0 flex justify-center items-center">
            <span className="font-black text-[9px] text-slate-700">
              {formatTime(timeLeft)}
            </span>
          </div>
        </div>
        ...
        <div className="flex-1">
          <div className="flex justify-between mb-1.5">
            <span className="max-w-[120px] font-bold text-slate-500 text-xs truncate">
              {quiz.title}
            </span>
      ...
      <div className="flex flex-col flex-1 mx-auto px-4 py-6 w-full max-w-2xl">
        <div
          className="bg-white shadow-indigo-100 shadow-lg mb-5 p-6 rounded-3xl"
          style={{
            opacity: animDir === "out" ? 0 : 1,
            transform:
              animDir === "out" ? "translateX(-30px)" : "translateX(0)",
        ...
        >
          ...
          <p className="font-black text-slate-800 text-lg md:text-xl leading-snug">
            {q.question}
          </p>
        </div>
          ...
          {q.options.map((opt, oi) => {
            ...
            return (
              <button
                ...
                style={{
                  transform:
                    isSelected && !confirmed ? "scale(1.02)" : "scale(1)",
                }}
              >
                <span
        ...
        <div className="mt-6">
          {!confirmed ? (
            ...
            <div
              className={`w-full py-4 rounded-2xl font-black text-base text-center ${
                Number(selected) === correctIdx
